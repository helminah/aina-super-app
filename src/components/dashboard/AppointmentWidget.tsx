    ...
    return (
      ...
      >
        ...
        <div className="flex-1">
          <p className="text-[11px] uppercase tracking-[0.15em] text-violet-600 font-semibold">Rendez-vous</p>
          <p className="font-heading font-bold text-bark-800 mt-0.5">Aucun rendez-vous prévu</p>
          <p className="text-xs text-bark-500 mt-0.5">Ajouter un premier rendez-vous médical</p>
        </div>
      ...
      <div className="bg-white rounded-2xl p-5 elev-2">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[11px] uppercase tracking-[0.15em] text-violet-700 font-semibold">Prochains RDV</p>
          </div>
          <button
            onClick={() => setShowAdd(true)}
...
function AppointmentRow({ appt, onRemove }: { appt: Appointment; onRemove: () => void }) {
  ...
  const label = daysAway === 0 ? 'Aujourd\'hui' : daysAway === 1 ? 'Demain' : `Dans ${daysAway} jours`;
    ...
    <div className="flex items-center gap-3 p-2 rounded-xl bg-ivory-100">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5 text-xs text-bark-500 mt-0.5">
          ...
          <span>·</span>
          <span>{date.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' })}</span>
      ...
      >
        <div className="p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-heading text-xl font-bold text-bark-800">Nouveau rendez-vous</h2>
          ...
          <div className="space-y-4">
            <div>
              <label className="text-sm text-bark-600 font-medium">Titre</label>
              <input
                type="text"
                value={title}
                onChange={e => setTitle(e.target.value)}
                placeholder="Ex. visite pédiatre 9 mois"
            ...
            <div>
              <label className="text-sm text-bark-600 font-medium">Type</label>
              <div className="flex gap-2 mt-1">
                {([
                  { id: 'checkup' as const, label: 'Visite', emoji: '👶' },
                  { id: 'vaccine' as const, label: 'Vaccin', emoji: '💉' },
                  { id: 'specialist' as const, label: 'Spécialiste', emoji: '🩺' },
                  ...
                  <button
                    ...
                    className={`flex-1 py-2.5 rounded-xl text-xs font-semibold flex flex-col items-center gap-0.5 ${
                      type === t.id ? 'bg-violet-500 text-white' : 'bg-ivory-200 text-bark-600'
                    }`}
                  >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-sm text-bark-600 font-medium">Date</label>
                <input
                  type="date"
                  value={date}
              ...
              <div>
                <label className="text-sm text-bark-600 font-medium">Heure</label>
                <input
                  type="time"
                  value={time}
            ...
            <div>
              <label className="text-sm text-bark-600 font-medium">Lieu (optionnel)</label>
              <input
                type="text"
                value={location}
                onChange={e => setLocation(e.target.value)}
                placeholder="Cabinet, hôpital, adresse…"
                className="w-full mt-1 px-4 py-3 rounded-xl bg-ivory-200 text-bark-800 focus:outline-none focus:ring-2 focus:ring-violet-300"
              />
            </div>
